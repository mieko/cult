  ...
  module CLI
    ...
    def node_cmd
      ...
      node_ssh = Cri::Command.define do
        ...
        run(arguments: 1..unlimited) do |opts, args, _cmd|
          ...
          Cult.paramap(nodes, concurrent: concurrent) do |node|
            ...
            File.chmod(0o600, node.ssh_private_key_file)

            ssh_args = 'ssh', '-i', esc.call(node.ssh_private_key_file),
                       '-p', esc.call(node.ssh_port.to_s),
                       '-o', "UserKnownHostsFile=#{esc.call(node.ssh_known_hosts_file)}",
                       esc.call("#{node.user}@#{node.host}")
            ssh_args += ssh_extra
      ...
      node_new = Cri::Command.define do
        ...
        required :r, :role, 'Specify possibly multiple /ROLE+/', multiple: true
        ...
        run(arguments: unlimited) do |opts, args, _cmd|
          ...
          names = opts[:count] ? generate_sequenced_names.call(names[0], opts[:count].to_i) : names
          ...
          Cult.paramap(nodes, concurrent: concurrent) do |node|
            if CLI.yes_no?("Destroy node `#{node}`?")
              ...
              begin
                ...
                puts "Exception while remote-destroying node: #{e}\n#{e.backtrace}"
      ...
      node_ls = Cri::Command.define do
        ...
        required :r, :role, 'List only nodes which include <value>', multiple: true

        run(arguments: unlimited) do |opts, args, _cmd|
          nodes = args.empty? ? Cult.project.nodes : CLI.fetch_items(args, from: Node)
          ...
          table = Terminal::Table.new(
            headings: ['Node', 'Provider', 'Zone', 'Public IPv4', 'Private IPv4', 'Roles']
          )
      ...
      node_sync = Cri::Command.define do
        ...
        required :R, :role, "Skip sync tasks not in /ROLE/.  Can be specified more than once.",
                 multiple: true

        required :P, :pass, "Only execute PASS.  Can be specified more than once.",
                 multiple: true

        run(arguments: unlimited) do |opts, args, _cmd|
          nodes = args.empty? ? Cult.project.nodes : CLI.fetch_items(args, from: Node)

          roles = if opts[:role].nil?
            Cult.project.roles
          else
      ...
      node_ping = Cri::Command.define do
        ...
        run(arguments: unlimited) do |_opts, args, _cmd|
          ...
          table = Terminal::Table.new(headings: %w(Node Status))
          table.rows = Cult.paramap(nodes, quiet: true) do |node|
            c = Commander.new(project: Cult.project, node: node)
            status = c.ping
            [node.name, status ? Rainbow(status).green : Rainbow("unreachable").red]
          end
          puts table
        end
      end
